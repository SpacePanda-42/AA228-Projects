...
def kernel_smoothing(state, q_matrix):
    """
    Run kernel smoothing to determine actions for states without observations

    Kwargs:
        State: state we are evaluating action for
        q_matrix: the q_matrix we generated from q_learning
    
    Returns:
        optimal_action: an integer corresponding to the optimal action calculated for the state
    """
    # epsilon = 0.01
    s_prime_list = []
    k_sum = 0
    n_states = q_matrix.shape[0]
    states_vec = np.arange(n_states-1) + 1
    
    s_prime_vec = abs(states_vec - state)
    for idx, val in enumerate(s_prime_vec):
        if val > 0:
            s_prime_vec[idx] = 1/val
    
    k_sum = np.sum(s_prime_vec)
    s_prime_vec = s_prime_vec/k_sum

    qas_list = []
    for action_vec in q_matrix.T[:,1:]:
        qas = np.multiply(s_prime_vec, action_vec)
        qas_list.append(qas)

    qas_arr = np.array(qas_list).T
    best_action = np.where(qas_arr == np.max(qas_arr))[1][0] # pick the first optimal action returned. Returns optimal action for no tie case, returns first action when there is a tie instead of picking randomly
...
def q_learn(model, n_iterations):
    ...
    # missing data choice 3: use kernel smoothing
    # missing_data = np.where(optimal_q[:,1]==0)[0]

    # print(f"Number of missing actions: {missing_data.shape[0]}")
    # i = 0
    # for state_idx in missing_data:
    #         i += 1
    #         if i % 50 == 0:
    #             print(f"solved {i}")
    #         optimal_q[state_idx][1] = kernel_smoothing(state_idx, q_array)
